...
const WorkScreen: React.FC = () => {
  ...
  const saveItems = async (itemsToSave: string[]) => {
    try {
      await AsyncStorage.setItem('workItems', JSON.stringify(itemsToSave));
    } catch (error) {
  ...
  const loadItems = async () => {
    try {
      const itemsJson = await AsyncStorage.getItem('workItems');
      if (itemsJson !== null) {
  ...
  const renderWorkItem = ({ item, index }: { item: string; index: number }) => {
    return (
